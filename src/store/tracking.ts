...
export const useTrackingStore = defineStore('tracking', {
    ...
    actions: {
        ...
        async fetchTrackingData(orderId: string, email: string, storeHash: string) {
            try {
                const response = await axios.get(`https://dbae-49-205-177-78.ngrok-free.app/track/order-shipment`, {
        ...
        async fetchTrackingDataByID(trackingId: string, storeHash: string) {
            try {
                const response = await axios.get(`https://dbae-49-205-177-78.ngrok-free.app/track/order-shipment-id`, {
